...
- (BOOL)matches:(NSString * _Nonnull)string SWIFT_WARN_UNUSED_RESULT;
@end
...
/// Note that, unlike the simpler wildcard form of <code>String.configurationPattern(for:)</code>, the regular expression is <em>not</em>
...
/// Note also that this implementation matches substrings. Include <code>^</code> and <code>$</code> if you want your pattern to match
/// against the entire URL.
- (BOOL (^ _Nonnull)(NSURL * _Nonnull))configurationPatternFor:(BOSService * _Nonnull)service SWIFT_WARN_UNUSED_RESULT;
/// :nodoc:
...
@property (nonatomic) BOOL defunctObserverCheckScheduled;
@property (nonatomic) NSInteger defunctObserverCheckCounter;
...
- (nonnull instancetype)initWithService:(BOSService * _Nonnull)service url:(NSURL * _Nonnull)url OBJC_DESIGNATED_INITIALIZER;
...
@interface BOSResource (SWIFT_EXTENSION(Siesta))
/// Matches this specific resource when passed as a pattern to <code>Service.configure(...)</code>.
- (BOOL (^ _Nonnull)(NSURL * _Nonnull))configurationPatternFor:(BOSService * _Nonnull)service SWIFT_WARN_UNUSED_RESULT;
/// :nodoc:
@property (nonatomic, readonly, copy) NSString * _Nonnull configurationPatternDescription;
@end
...
@property (nonatomic, readonly) BOOL beingObserved;
- (void)notifyObserversWithProgress:(double)progress;
@end
...
@property (nonatomic, readonly) uint64_t configVersion;
